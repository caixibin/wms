        ...
        $(function () {
                ...
                $.dialog.open(productListUrl, {
                    ...
                    close: function () {
                        ...
                        if (data != null) {
                            ...
                            currentTr.find("[tag=costPrice]").val(data["costPrice"]);//成本价
                        }
                    }
                });
            }).on("change", "[tag=costPrice],[tag=number]", function () {//计算amount
                //获取当前行,具有单独的一行
                var currentTr = $(this).closest("tr");
            ...
            $(".submit_btn").click(function(){
                $("#edit_table_body tr").each(function(index,item){
                    var _item = $(item);
                    _item.find("[tag=pid]").prop("name","stockIncomeBill.items["+index+"].product.id");
                    _item.find("[tag=costPrice]").prop("name","stockIncomeBill.items["+index+"].costPrice");
                    _item.find("[tag=number]").prop("name","stockIncomeBill.items["+index+"].number");
                    _item.find("[tag=remark]").prop("name","stockIncomeBill.items["+index+"].remark");
                });
                $("#editForm").submit();
            });
        });
            ...
            <table cellspacing="0" cellpadding="0" width="100%" align="left" border="0">
                ...
                <tr>
                    ...
                    <td class="ui_text_lt">
                        ...
                        <input class="ui_input_txt02 Wdate" name="stockIncomeBill.vdate"
                            ...
                            <s:else>
                                <s:iterator value="stockIncomeBill.items">
                                    <tr>
                                        ...
                                        <td><s:textfield tag="costPrice" name="costPrice" cssClass="ui_input_txt02"/></td>
