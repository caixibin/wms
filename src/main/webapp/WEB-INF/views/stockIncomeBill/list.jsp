				...
				<div class="ui_text_indent">
					<div id="box_border">
						...
						<div id="box_bottom">
							...
							<input type="button" value="新增" class="ui_input_btn01 btn_input" data-inputurl="<s:url action="stockIncomeBill_input"/>"/>
							<input type="button" value="删除所有" class="ui_input_btn01 btn_all" data-deleurl="<s:url action="stockIncomeBill_batchDelete"/>"/>
						</div>
					</div>
				</div>
			</div>
								...
								<tr>
									...
									<td>
										<s:if test="status == 0">
											...
											<s:a action="stockIncomeBill_input">编辑
												<s:param name="stockIncomeBill.id" value="id"/>
											</s:a>
											<s:url namespace="/" action="stockIncomeBill_delete" var="deleUrl" escapeAmp="false">
												<s:param name="stockIncomeBill.id" value="id"/>
											</s:url>
											<a href="javascript:;" class="btn_delete" data-deleurl="<s:property value="#deleUrl"/>">删除</a>
										</s:if>
										<s:else>
											<s:a action="stockIncomeBill_view">查看
												<s:param name="stockIncomeBill.id" value="id"/>
											</s:a>
										</s:else>
									</td>
